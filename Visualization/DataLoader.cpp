...
NodeId DataLoader::decodeNodeId(const std::string& encoded) {
    ...
    if (colon == std::string::npos)
        throw std::runtime_error("DataLoader: malformed NodeId (no colon): " + encoded);
    ...
    try {
        ...
        throw std::runtime_error("DataLoader: malformed NodeId hex value: " + encoded);
    }
        ...
        if (key == "VISDATA_VERSION") {
            ...
            if (version != FORMAT_VERSION)
                throw std::runtime_error(
                    "DataLoader: incompatible file version " +
...
void DataLoader::parseContigs(std::istream& in, VisSession& session,
    ...
    if (line != "BEGIN_CONTIGS")
        throw std::runtime_error("DataLoader: expected BEGIN_CONTIGS, got: " + line);
    ...
    while (std::getline(in, line)) {
        ...
        if (key == "CONTIG") {
            ...
            if (seqLine.substr(0, 4) != "SEQ ")
                throw std::runtime_error(
                    "DataLoader: expected SEQ after CONTIG " +
                    std::to_string(index));
...
void DataLoader::parseScaffolds(std::istream& in, VisSession& session,
    ...
    if (line != "BEGIN_SCAFFOLDS")
        throw std::runtime_error("DataLoader: expected BEGIN_SCAFFOLDS, got: " + line);
...
void DataLoader::parseNodes(std::istream& in, VisSession& session,
    ...
    std::getline(in, line);
    if (line != "BEGIN_NODES")
        throw std::runtime_error("DataLoader: expected BEGIN_NODES, got: " + line);
...
void DataLoader::parseEdges(std::istream& in, VisSession& session,
    ...
    std::getline(in, line);
    if (line != "BEGIN_EDGES")
        throw std::runtime_error("DataLoader: expected BEGIN_EDGES, got: " + line);
...
void DataLoader::parseContigSteps(std::istream& in, VisSession& session,
    ...
    if (line != "BEGIN_CONTIG_STEPS")
        throw std::runtime_error("DataLoader: expected BEGIN_CONTIG_STEPS, got: " + line);
...
void DataLoader::parseEulerSteps(std::istream& in, VisSession& session,
    ...
    if (line != "BEGIN_EULER_STEPS")
        throw std::runtime_error("DataLoader: expected BEGIN_EULER_STEPS, got: " + line);
...
VisSession DataLoader::load(const std::string& filePath) {
    ...
    if (!in.is_open())
        throw std::runtime_error("DataLoader: could not open file: " + filePath);
